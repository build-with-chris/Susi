...
import { addVideoToProject } from "./actions";
...
export function UploadVideosToProject({
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!files?.length) {
      setError("Bitte mindestens ein Video auswählen.");
      return;
    }
  ...
  return (
    <div className="mt-8">
      ...
      <form onSubmit={handleUpload} className="mt-6">
        <label htmlFor="video-files" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
          Videos auswählen (MP4, MOV, WebM)
        </label>
        <input
          id="video-files"
          name="files"
        ...
        <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
          Die Videos werden im Browser zu 720p60 (MP4) konvertiert und anschließend zu Supabase hochgeladen. Beim
          ersten Upload wird FFmpeg (~31 MB) geladen.
        </p>
        <div className="mt-4 flex items-center gap-4">
          ...
          >
            {uploading ? "Wird hochgeladen…" : "Videos hochladen"}
          </button>
          {uploadProgress && (
