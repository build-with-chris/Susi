...
-- 9) Bewertung, Postingdatum und Kommentare für Bilder (wie bei Videos)
...
DROP POLICY IF EXISTS "Allow public read on project_image_comments" ON public.project_image_comments;
CREATE POLICY "Allow public read on project_image_comments"
  ON public.project_image_comments FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public insert on project_image_comments" ON public.project_image_comments;
CREATE POLICY "Allow public insert on project_image_comments"
  ON public.project_image_comments FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public delete on project_image_comments" ON public.project_image_comments;
